...
def simulate_data(output_path):
    ...
    db = client["scoring"]
    collection_user_data = db["simulated_USER_DATA_with_dates"]
    collection_user_kyc = db["simulated_KYC_DATA"]
    collection_real_transactions = db["real_transactions_with_dates"]
    
    batch_size = 100000  # Choisissez une taille de lot Pour insertion par lot
    ...
    transactions_d = transactions_df.to_dict("records")
    collection_real_transactions.delete_many({})
    for i in range(0, len(transactions_d), batch_size):
        batch = transactions_d[i:i + batch_size]
        collection_real_transactions.insert_many(batch)
    #collection_real_transactions.insert_many(transactions_d)
    print("Simulation des données réel de transactions avec de dates...")
    ...
    data_u = user_data.to_dict("records")
    collection_user_data.delete_many({})
    
    for i in range(0, len(data_u), batch_size):
        batch = data_u[i:i + batch_size]
        collection_user_data.insert_many(batch)
    ...
    data_kyc = kyc_data.to_dict("records")
    collection_user_kyc.delete_many({})
    for i in range(0, len(data_kyc), batch_size):
        batch = data_kyc[i:i + batch_size]
        collection_user_kyc.insert_many(batch)
    #collection_user_kyc.insert_many(data_kyc)
    del data_kyc
    gc.collect()
    ...
    return "Simulation terminée." #[transactions_with_dates_csv, transactions_with_dates_parquet, user_data_csv, user_data_parquet, kyc_data_csv, kyc_data_parquet]
